...
const Spreadsheet = () => {
  ...
  useEffect(() => {
    const unsubscribe = firebase.firestore().collection('AI3Spares').onSnapshot((snapshot) => {
  ...
  const handleAddItem = async () => {
    try {
      const newItemRef = await firebase.firestore().collection('AI3Spares').add(newItem);
  ...
  const handleDeleteItem = async (itemId) => {
    try {
      await firebase.firestore().collection('AI3Spares').doc(itemId).delete();
  ...
  const handleConfirm = async () => {
    try {
      ...
      items.forEach((item) => {
        const itemRef = firebase.firestore().collection('AI3Spares').doc(item.id);
  ...
  const PressDelete = async () => {
    try {
      const collectionRef = firebase.firestore().collection('AI3Spares');
