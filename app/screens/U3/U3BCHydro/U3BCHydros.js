...
const SummaryPage = ({ navigation }) => {
  ...
  const pressBCHydro1Handler = () => {
    navigation.navigate('U3BCHydro1Spares');
  };
  const pressBCHydro2Handler = () => {
    navigation.navigate('U3BCHydro2Spares');
  };
  const pressBCHydro3Handler = () => {
    navigation.navigate('U3BCHydro3Spares');
  };
  const pressBCHydro4Handler = () => {
    navigation.navigate('U3BCHydro4Spares');
  };
  ...
  useEffect(() => {
    const fetchCSData = async () => {
      const querySnapshotBCHydro2CS = await firebase.firestore().collection('BCHydro1Spares')
        .get();

      const querySnapshotBCHydro1CS = await firebase.firestore().collection('BCHydro2Spares')
