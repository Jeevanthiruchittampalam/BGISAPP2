...
const SummaryPage = ({ navigation }) => {
  ...
  const pressAI1Handler = () => {
    navigation.navigate('U3AI1Spares');
  };
  const pressAI2Handler = () => {
    navigation.navigate('U3AI2Spares');
  };
  const pressAI3Handler = () => {
    navigation.navigate('U3AI3Spares');
  };
  const pressAI4Handler = () => {
    navigation.navigate('U3AI4Spares');
  };
  ...
  useEffect(() => {
    const fetchCSData = async () => {
      const querySnapshotAI2CS = await firebase.firestore().collection('AI1Spares')
        .get();

      const querySnapshotAI1CS = await firebase.firestore().collection('AI2Spares')
