...
const Spreadsheet = () => {
  ...
  useEffect(() => {
    const unsubscribe = firebase.firestore().collection('ABSpares').onSnapshot((snapshot) => {
  ...
  const handleAddItem = async () => {
    try {
      const newItemRef = await firebase.firestore().collection('ABSpares').add(newItem);
  ...
  const handleDeleteItem = async (itemId) => {
    try {
      await firebase.firestore().collection('ABSpares').doc(itemId).delete();
  ...
  const handleConfirm = async () => {
    try {
      ...
      items.forEach((item) => {
        const itemRef = firebase.firestore().collection('ABSpares').doc(item.id);
  ...
  const PressDelete = async () => {
    try {
      const collectionRef = firebase.firestore().collection('ABSpares');
