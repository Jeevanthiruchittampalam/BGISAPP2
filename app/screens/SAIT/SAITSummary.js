...
const SummaryPage = ({ navigation }) => {
  ...
  const pressSAIT2Handler = () => {
    navigation.navigate('SAIT2Spares');
  };
  const pressSAIT1Handler = () => {
    navigation.navigate('SAIT1Spares');
  };
  const pressSAIT3Handler = () => {
    navigation.navigate('SAIT3Spares');
  };
  const pressSAIT4Handler = () => {
    navigation.navigate('SAIT4Spares');
  };
  ...
  useEffect(() => {
    const fetchCSData = async () => {
      const querySnapshotSAIT1CS = await firebase.firestore().collection('SAIT1Spares')
        .get();

      const querySnapshotSAIT2CS = await firebase.firestore().collection('SAIT2Spares')
  ...
  const fullTotalNo = abTotalNoSAIT4 + abTotalNoSAIT1 + abTotalNoSAIT2 + abTotalNoSAIT3;

  const fullTotalYes = abTotalYesSAIT4 + abTotalYesSAIT1 + abTotalYesSAIT2 +abTotalNoSAIT3;
  ...
  return (
    <ScrollView style={styles.container}>
      <View>
        ...
        <TouchableOpacity style={[styles.button, { backgroundColor: 'orange' }]} onPress={pressSAIT1Handler} >
        ...
        <View style={styles.tableContainer}>
        <TouchableOpacity style={[styles.button, { backgroundColor: '#39FF14' }]}onPress={pressSAIT2Handler}>
